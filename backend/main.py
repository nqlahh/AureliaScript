...
def generate_diagram(request: DocsRequest):
    """
    Generates a Mermaid.js syntax Class Diagram for the codebase.
    """
    if rag_instance.index is None:
        raise HTTPException(status_code=400, detail="No codebase uploaded yet.")

    # Search for class definitions
    query = "class definitions and architecture"
    context, sources = rag_instance.search_code(query, top_k=5)

    if not context:
        return {"diagram_syntax": "classDiagram\n  %% No code found"}

    # Prompt Engineering for Mermaid Class Diagram
    prompt = f"""
    You are a Software Architect. Generate a Mermaid.js Class Diagram syntax based on the provided Python code.
    
    REQUIREMENTS:
    1. Output ONLY the Mermaid code. Do not include explanations.
    2. Use 'classDiagram' keyword.
    3. Define classes, methods, and attributes.
    4. Use arrows (--> or ..>) for relationships.
